...
/**
 * Dynamic offset namespace that auto-detects and caches offsets
 * Falls back to hardcoded values if detection fails
 */
class OffsetManager {
private:
    static OffsetDetector::OffsetCache currentOffsets;
    static bool initialized;

public:
    static void Initialize(HANDLE processHandle, const std::string& robloxVersion) {
        if (!initialized) {
            currentOffsets = OffsetDetector::DetectOffsets(processHandle, robloxVersion);
            OffsetDetector::LogOffsets(currentOffsets);
            initialized = true;
        }
    }
...
// Global offset accessor - used throughout the code
namespace offsets {
    // STATIC FALLBACK - In case manager isn't initialized yet
    // Instance
    ...
    // Get dynamic offsets if available
    inline std::uint64_t GetName() {
        try {
            return OffsetManager::GetOffsets().Name;
        } catch (...) {
            return Name;
        }
    }

    inline std::uint64_t GetChildren() {
        try {
            return OffsetManager::GetOffsets().Children;
        } catch (...) {
            return Children;
        }
    }

    inline std::uint64_t GetParent() {
        try {
            return OffsetManager::GetOffsets().Parent;
        } catch (...) {
            return Parent;
        }
    }

    inline std::uint64_t GetClassDescriptor() {
        try {
            return OffsetManager::GetOffsets().ClassDescriptor;
        } catch (...) {
            return ClassDescriptor;
        }
    }

    inline std::uint64_t GetLocalPlayer() {
        try {
            return OffsetManager::GetOffsets().LocalPlayer;
        } catch (...) {
            return LocalPlayer;
        }
    }

    inline std::uint64_t GetLocalScriptEmbedded() {
        try {
            return OffsetManager::GetOffsets().LocalScriptEmbedded;
        } catch (...) {
            return LocalScriptEmbedded;
        }
    }

    inline std::uint64_t GetModuleScriptEmbedded() {
        try {
            return OffsetManager::GetOffsets().ModuleScriptEmbedded;
        } catch (...) {
            return ModuleScriptEmbedded;
        }
    }
}
